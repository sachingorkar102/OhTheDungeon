...
//                            (new TFGenCanopyTree()).generate(world, rand, new BlockPos(mdx, dy, mdz));
     ...
     */
    public void copyToStructure(AsyncWorldEditor world, int dx, int dy, int dz, StructureTFComponentOld component, StructureBoundingBox sbb) {
        for (int x = 0; x < rawWidth; x++) {
            for (int z = 0; z < rawDepth; z++) {
...
//                            (new TFGenCanopyTree()).generate(world, rand, mdx, dy, mdz);
        ...
        for (int x = 0; x < rawWidth; x++) {
            for (int z = 0; z < rawDepth; z++) {
                if (getRaw(x, z) == 0) {
                    ...
                    if (isEven(x) && isEven(z)) {
                        if (shouldTorch(x, z) && component.getBlockStateFromPos(world, mdx, mdy, mdz, sbb) == wallBlockState.getMaterial()) {
    ...
    private void makeWallThing(AsyncWorldEditor world, int dy, StructureTFComponentOld component, StructureBoundingBox sbb, int mdx, int mdz, int even, int odd) {
        for (int y = 0; y < head; y++) {
            putHeadBlock(world, mdx + even, dy + tall + y, mdz + odd, component, sbb);
        }
        for (int y = 0; y < tall; y++) {
     ...
     */
    private void putPillarBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component, StructureBoundingBox sbb) {
     ...
     */
    private void putWallBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component, StructureBoundingBox sbb) {
        if (wallBlocks != null) {
     ...
     * Puts a wall block in the structure, at the specified structure coordinates.
     */
    private void putDoorBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component, StructureBoundingBox sbb) {
    ...
    private void putHeadBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component, StructureBoundingBox sbb) {
     ...
     */
    private void putRootBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component, StructureBoundingBox sbb) {
     ...
     */
    private void putCanopyTree(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component, StructureBoundingBox sbb) {
