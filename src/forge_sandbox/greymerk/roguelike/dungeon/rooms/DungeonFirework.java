...
import forge_sandbox.greymerk.roguelike.worldgen.redstone.Comparator;
...
import forge_sandbox.greymerk.roguelike.worldgen.redstone.Lever;
import forge_sandbox.greymerk.roguelike.worldgen.redstone.Repeater;
...
public class DungeonFirework extends DungeonBase {
	...
	private void launcher(IWorldEditor editor, Random rand, Cardinal dir, Coord pos){
		...
		for(int i = 0;i < 8; ++i){
			...
			stick = Loot.setItemNameNew(stick, Integer.toString(i));
			stick = Loot.setItemLoreNew(stick, "Random logic unit", TextFormat.DARKGRAY);
		...
		while(top.getY() > pos.getY()){
			...
			if(editor.getMaterial(top).isSolid()) break;
		}
