...
import forge_sandbox.com.someguyssoftware.dungeons2.graph.Waypoint;
import forge_sandbox.com.someguyssoftware.dungeons2.graph.mst.Edge;
import forge_sandbox.com.someguyssoftware.dungeons2.graph.mst.EdgeWeightedGraph;
import forge_sandbox.com.someguyssoftware.dungeons2.graph.mst.LazyPrimMST;
...
//        
        ...
        if (factor < 1.0D) {
            ...
            Dungeons2.log.debug("deltaX -> {}", new Object[] {
                            deltaX
                        });
            Dungeons2.log.debug("deltaZ -> {}", new Object[] {
                            deltaZ
                        });
            int xAmount = (int) ((deltaX * (1.0 - factor)) / 2);
            int zAmount = (int) ((deltaZ * (1.0 - factor)) / 2);
            Dungeons2.log.debug("initial shrink amounts -> {} {}", new Object[] {xAmount, zAmount});
            ...
            if (Math.abs(deltaX - (xAmount*2)) < MIN_BOUNDARY_SIZE ) {
                ...
                Dungeons2.log.debug("x less than min, new amount -> {} [{}percent]", new Object[] {xAmount, p});
            }
            if (Math.abs(deltaZ - (zAmount*2)) < MIN_BOUNDARY_SIZE) {
                ...
                Dungeons2.log.debug("z less than min, new amount -> {} [{}percent]", new Object[] {zAmount, p});
            }
            
            AxisAlignedBB newBoundary = boundary.grow(-xAmount, 0, -zAmount);
            Dungeons2.log.debug("boundary shrunk by -> {} {}, to new size -> {}", new Object[] {xAmount, zAmount, newBoundary});
            return newBoundary;
        }
...
//        Dungeons2.log.info("BufferedRooms.size=:" + bufferedRooms.size());
    ...
    public Shaft join(Level sourceLevel, Level destLevel) {
        ...
        if (destRooms == null || sourceRooms == null || destRooms.isEmpty() || sourceRooms.isEmpty()) return shaft;
     ...
     * @return
     */
    public Level build(AsyncWorldEditor world, Random rand, ICoords startPoint, List<Room> plannedRooms, ILevelConfig config) {
        ...
        Dungeons2.log.debug("Before Apply Distance Buffering Rooms.size -> {}", new Object[] {
                    anchors.size() + spawned.size()
                });
        rooms = applyDistanceBuffering(rand, startPoint, anchors, spawned);
        Dungeons2.log.debug("After Apply Distance Buffering Rooms.size -> {}, room loss -> {}", new Object[] {rooms.size(), getRoomLossToDistanceBuffering()});
//        System.out.println("After Apply Distance Buffering Rooms.size -> {}, room loss -> {}", rooms.size(), getRoomLossToDistanceBuffering());
        ...
        Dungeons2.log.debug("After select valid rooms Rooms.size -> {}, room loss -> {}", new Object[] {rooms.size(), getRoomLossToValidation()});
        if (rooms.size() < MIN_NUMBER_OF_ROOMS) {
            return EMPTY_LEVEL;
        }
...
//            midpoint.resetZ(midpoint.getZ()/2);
            ...
            minXMap.put(room1.getMinX(), room1);
            minXMap.put(room2.getMinX(), room2);
            
            Map<Integer, Room> maxXMap = new HashMap<>(5);
            maxXMap.put(room1.getMaxX(), room1);
            maxXMap.put(room2.getMaxX(), room2);
            
            Map<Integer, Room> minZMap = new HashMap<>(5);
            minZMap.put(room1.getMinZ(), room1);
            minZMap.put(room2.getMinZ(), room2);
            
            Map<Integer, Room> maxZMap = new HashMap<>(5);
            maxZMap.put(room1.getMaxZ(), room1);
            maxZMap.put(room2.getMaxZ(), room2);            
...
//                Dungeons2.log.info("Skipped V termination point altogether somehow!");
     ...
     * @return
     */
    protected List<Room> selectValidRooms(AsyncWorldEditor world, Random rand, List<Room> rooms, ILevelConfig config) {
        ...
        for (Room room : rooms) {
            ...
            if (isValid &&
                ...
                Dungeons2.log.debug("Removing room for being outside dungeon bounds -> {}", new Object[] {
                                    room.getId()
                                });
...
//                    Dungeons2.log.debug("room[{}] is NOT valid at chuck(s) -> {}", room.getId(), room.getXZCenter());
                    Dungeons2.log.debug("Removing room for residing in unloaded chunk -> {}", new Object[] {
                                            room.getId()
                                        });
                    incrementLossToValidation(1);
                }
            ...
            if (isValid) {
                ...
                Dungeons2.log.debug("Removing room for failing constraints -> {}", new Object[] {
                                    room
                                });
                incrementLossToValidation(1);
            }
...
//    }
     ...
     * @return
     */
    public boolean isRoomInLoadedChunks(AsyncWorldEditor world, Room room) {
        return true;
    }
     ...
     * @return
     */
    protected boolean validateRoomConstraints(AsyncWorldEditor world, Room room, ILevelConfig config) {
        ...
        if (room.getCoords().getY() <= getDungeonBuilder().getConfig().getBottomLimit()) {
            if (Dungeons2.log.isDebugEnabled()) {
                Dungeons2.log.debug("Room bottom [{}] is below min y constraint [{}]", 
                        new Object[] {room.getCoords().getY(), 
...
//        }
        ...
        if (room.getCoords().getY() + room.getHeight() > 63) {
            
            Dungeons2.log.trace("Room is above sea level -> {}", new Object[] {room.getCenter()});
            /*
...
//    }
     ...
     * @return
     */
    protected Room buildStartRoom(AsyncWorldEditor world, Random rand, AxisAlignedBB field, ICoords startPoint, ILevelConfig config) {
        /*
         * the start of the level
         */
        Room startRoom = new Room().setStart(true).setAnchor(true).setType(Type.LADDER);
...
//    }
     ...
     * @return
     */
    protected Room buildEndRoom(AsyncWorldEditor world, Random rand,
...
//    }
     ...
     * @return
     */
    protected Room buildPlannedRoom(AsyncWorldEditor world, Random rand, AxisAlignedBB field, 
...
//    }
     ...
     * @return
     */
    protected Room buildBossRoom(AsyncWorldEditor world, Random rand, AxisAlignedBB field,
...
//    }
     ...
     */
    @Deprecated
    protected Room buildEntranceRoom(AsyncWorldEditor world, Random rand, ICoords surfaceCoords, Room startRoom,
