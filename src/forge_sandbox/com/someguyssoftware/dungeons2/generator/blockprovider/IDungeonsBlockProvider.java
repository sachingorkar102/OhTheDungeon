...
public interface IDungeonsBlockProvider {
    ...
    default public BlockData getBlockState(Arrangement arrangement, Style style, int decayIndex) {
        ...
        if (block == null || "".equals(block)) return blockState;
        ...
        if (elem == DesignElement.CROWN || elem == DesignElement.TRIM ||
             ...
             */
            Direction d;
            if (false/*blockState instanceof Directional*/) {
...
//            Dungeons2.log.debug("style=" + style);
    ...
    default public boolean isWallElement(ICoords coords, Room room, Layout layout) {
        return coords.getX() == room.getMinX() || coords.getZ() == room.getMinZ() ||
                coords.getX() == room.getMaxX() || coords.getZ() == room.getMaxZ();
    }
    ...
    default public boolean isPilasterElement(ICoords coords, Room room, Layout layout) {
        ...
        return ((x == room.getMinX()+1 || x == room.getMaxX()-1) && Math.abs(zIndex % 2) == 1 && zIndex > 0 && zIndex < room.getDepth() - 1) ||
                ((z == room.getMinZ()+1 || z == room.getMaxZ()-1) && Math.abs(xIndex % 2) == 1 && xIndex > 0 && xIndex < room.getWidth() - 1);
    }
