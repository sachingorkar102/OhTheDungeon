...
public class Boundary {
	...
	public Boundary resize(final double factor, final int minSize) {
		...
		if (factor < 1.0D) {
			...
			Dungeons2.log.debug("deltaX -> {}", new Object[] {deltaX});
			Dungeons2.log.debug("deltaZ -> {}", new Object[] {deltaZ});
			...
			if (Math.abs(deltaX - (xAmount*2)) < minSize ) {
				...
				Dungeons2.log.debug("x less than min, new amount -> {} [{}%]", new Object[] {xAmount, p});
			}
			if (Math.abs(deltaZ - (zAmount*2)) < minSize) {
				...
				Dungeons2.log.debug("z less than min, new amount -> {} [{}%]", new Object[] {zAmount, p});
			}
