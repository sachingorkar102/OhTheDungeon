package forge_sandbox.jaredbgreat.dldungeons.setup;
...
public class Externalizer {
	...
	private static final   String baseLocation = "/forge_sandbox/jaredbgreat/dldungeons/res/themes/";
		...
		try {
			...
			if((instream != null) && (outstream != null)) 
				while((line = instream.readLine()) != null) {
					...
					System.err.println("[OTD] Error! Failed to write theme file " + outFile);
				}
		...
		try {
			...
			if(outFile.exists()) {
				if(outFile.canWrite() && !outFile.isDirectory()) {
					...
					System.err.println("[OTD] Warning, could not delete " + outFile);
					result = false;
				}
			}
			instream = new BufferedReader(new InputStreamReader(getClass()
	...
	public void makeThemes() {
		for(String name : themes) {
			System.out.println("[OTD] Installing file " + outDirectory + name);
			copyOut(name);
		}
	}
	...
	public void makeChestCfg() {
		System.out.println("[OTD] Installing files "	+ outDirectory + "chests.cfg");
	...
	public void makeNBTCfg() {
		System.out.println("[OTD] Installing files " + outDirectory + "nbt.cfg and " );
		copyOut("nbt.cfg");
	}
