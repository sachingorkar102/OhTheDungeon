...
public class TreasureChest extends BasicChest {
	...
	public TreasureChest(int x, int y, int z, int level, LootCategory category) {
        ...
        private static void addToInventory(Block chest, int index, ItemStack item) {
            Chest bih = (Chest) chest.getState();
            bih.getInventory().setItem(index, item);
//            bih.update(true, false);
	...
	public void place(World world, int x, int y, int z, Random random) {
		...
		ItemStack treasure;
		if(world.getBlockAt(pos).getType() != DBlock.chest) {
